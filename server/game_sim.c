...
void world_reset(world_t *w) {
    ...
    for (int s = 0; s < 3; s++) {
        ...
        /* Emit placeholder RARITY_TIER events (kind=2) for each tier.
         * The actual tier messages will be generated by scripts/generate-motd.sh
         * and stored in `stations/{slug}/motd.json` on S3. These seed events
         * establish the chain structure and can be updated as tier messages change. */
        const char *tier_names[] = { "common", "uncommon", "rare", "ultra_rare" };
        for (int tier_idx = 0; tier_idx < 4; tier_idx++) {
            const char *placeholder_text = tier_names[tier_idx];
            int tier_text_len = (int)strlen(placeholder_text);
            chain_payload_operator_post_t tier_event = {
                .kind = 2,                  /* RARITY_TIER */
                .tier = (uint8_t)tier_idx,  /* 0=common,1=uncommon,2=rare,3=ultra_rare */
                .ref_id = 0,                /* motd seed */
                .text_len = (uint16_t)tier_text_len,
            };
            sha256_bytes((const uint8_t *)placeholder_text, tier_text_len, tier_event.text_sha256);
            uint8_t tier_payload[38 + 256];
            memcpy(tier_payload, &tier_event, 38);
            if (tier_text_len > 0) {
                memcpy(tier_payload + 38, placeholder_text, (size_t)tier_text_len);
            }
            (void)chain_log_emit(w, st, CHAIN_EVT_OPERATOR_POST, tier_payload, (uint16_t)(38 + tier_text_len));
        }
    }
